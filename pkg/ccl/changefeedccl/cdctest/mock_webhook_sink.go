...
// StartMockWebhookSinkWithTLS creates and starts a mock Webhook sink for tests with
// TLS enabled.
func StartMockWebhookSinkWithTLS(certificate *tls.Certificate) (*MockWebhookSink, error) {
...
// authentication.
func (s *MockWebhookSink) EnableBasicAuth(username string, password string) {
	s.basicAuth = true
	s.username = username
	s.password = password
}
